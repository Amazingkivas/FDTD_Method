...
Test_FDTD::Test_FDTD(Parameters _parameters, bool _shifted) : shifted(_shifted)
{
	parameters = _parameters;
}

void Test_FDTD::initial_filling(FDTD& _test, SelectedFields fields, std::function<double(double, double[2])>& init_function)
{
...
double Test_FDTD::get_max_abs_error(Field& this_field, Component field, std::function<double(double, double, double[2])>& true_function, double time)
{
	double shift = 0.0;
	...
	if (axis == Axis::X)
	{
		shift = get_shift(field, parameters.dx);
		double x = (shifted) ? shift : 0.0;
		double size_x[2] = { parameters.ax, parameters.bx };
		int j = 0;
		int k = 0;
		for (int i = 0; i < this_field.get_Ni(); ++i, x += parameters.dx)
		{
	...
	{
		shift = get_shift(field, parameters.dy);
		double y = (shifted) ? shift : 0.0;
		double size_y[2] = { parameters.ay, parameters.by };
		int i = 0;
		int k = 0;
		for (int j = 0; j < this_field.get_Nj(); ++j, y += parameters.dy)
		{
	...
	{
		shift = get_shift(field, parameters.dz);
		double z = (shifted) ? shift : 0.0;
		double size_z[2] = { parameters.az, parameters.bz };
		int i = 0;
		int j = 0;
		for (int k = 0; k < this_field.get_Nk(); ++k, z += parameters.dz)
		{
