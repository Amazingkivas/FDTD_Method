...
class ComputeSigmaFunctor {
    ...
    public:
        ...
        KOKKOS_INLINE_FUNCTION void operator()(const int& k, const int& j, const int& i) const {
            ...
            Esigma[index] = SGm * std::pow(static_cast<double>(dist(i, j, k))
               / static_cast<double>(pml_size), FDTD_const::N);
            Bsigma[index] = SGm * std::pow(static_cast<double>(dist(i, j, k))
               / static_cast<double>(pml_size), FDTD_const::N);
        }
    };
    ...
    public:
        ...
        static void apply(Field Ex, Field Ey, Field Ez,
            ...
            Kokkos::MDRangePolicy<Kokkos::Rank<3>> policy({bounds_k.first, bounds_j.first, bounds_i.first},
